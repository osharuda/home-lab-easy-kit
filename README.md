...
`Customizer` generates C and C++ headers required for firmware and software parts. Also customizer is capable to detect some issues in JSON configuration and to warn user.
...
`Software` part consists of C++ library based on JSON configuration file provided to Customizer. Also, there is monitor tool linked with this library that can be used to test software to firmware communication and firmware peripherals being used.
