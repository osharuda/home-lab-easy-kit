...
const UartProxyDevInstance g_uart_proxy_descriptors[]=UART_PROXY_DESCRIPTOR;

UARTDev::UARTDev(std::shared_ptr<EKitBus>& ebus, int addr) : super(ebus, addr)  {
    static const char* const func_name = "UARTDev::UARTDev";
    descr = FindDeviceDescriptor(g_uart_proxy_descriptors, addr);
...
std::string UARTDev::get_dev_name() const {
    return descr->dev_name;
}

const UartProxyDevInstance* UARTDev::get_descriptor() const {
    return descr;
}
