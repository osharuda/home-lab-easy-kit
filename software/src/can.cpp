...
/// \brief Global array that stores all virtual CanDev devices configurations.
const CanInstance g_can_descriptors[] = CAN_SW_DEV_DESCRIPTOR;
/// @}

const CanInstance* CanDev::get_descriptor(size_t index) {
	if (index<CAN_DEVICE_COUNT) {
		return const_cast<CanInstance*>(g_can_descriptors + index);
	} else {
		assert(false);
	}
...
std::string CanDev::get_dev_name() const {
	return descr->dev_name;
}

CanDev::CanDev(std::shared_ptr<EKitBus>& ebus, int addr) : super(ebus, addr) {
	static const char* const func_name = "CanDev::CanDev";
	for (int i=0; i<CAN_DEVICE_COUNT; i++) {
		if (addr==g_can_descriptors[i].dev_id) {
			descr = g_can_descriptors + i;
			break;
		}
	}

	if (descr==nullptr) {
		throw EKitException(func_name, EKIT_BAD_PARAM, "addr specified doesn't correspond to any of Can devices");
	}
}
