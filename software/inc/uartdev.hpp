...
class UARTDev final : public EKitVirtualDevice {
    ...
    const UartProxyDevInstance* descr = nullptr;    ///< Pointer to the device descriptor structure.
public:
    ...
    ///        to the UARTDev device.
    UARTDev(std::shared_ptr<EKitBus>& ebus, int addr);
    ...
    /// \brief Returns structure describing UARTDev device.
    /// \return Pointer to the UartProxyDevInstance structure.
    const UartProxyDevInstance* get_descriptor() const;
};
