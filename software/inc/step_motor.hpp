...
class StepMotorDev final : public EKitVirtualDevice{
    ...
    /// \brief Pointer to the #tag_StepMotorDevice structure that describes StepMotorDev virtual device represented by this class.
	const StepMotorDevice* descr = nullptr;
    ...
    /// \brief Returns virtual device descriptor.
    /// \param index - virtual device index. Must be in range between 0 to #SW_STEP_MOTOR_DEVICE_COUNT-1.
    /// \return Pointer to the tag_StepMotorDevice structure with description of the virtual device.
    static const PStepMotorDevice get_descriptor(size_t index);
    ...
    ///        to the StepMotorDev device.
	StepMotorDev(std::shared_ptr<EKitBus>& ebus, int addr);
...
#endif // STEP_MOTOR_DEVICE_ENABLED
