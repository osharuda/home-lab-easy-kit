...
public:
	...
	EKitDeviceBase(std::shared_ptr<EKitBus>& ebus, int addr) : bus(ebus), dev_addr(addr) {
	}
	...
	virtual int get_addr() const {
		return dev_addr;
	}

    /// \brief Returns device name.
    /// \return String with name. By default name is device address converted to string, to get meaningful value
    ///         this call must be overridden.
	virtual std::string get_dev_name() const {
		return std::to_string(dev_addr);
	}
};
...
public:
    ...
    /// \param addr - device id of the virtual device to be attached to the instance of this class.
	EKitVirtualDevice(std::shared_ptr<EKitBus>& ebus, int addr) : EKitDeviceBase(ebus, addr) {
		static const char* const func_name = "EKitVirtualDevice::EKitVirtualDevice";
		int busid = 0;
		bus->bus_props(busid);
		if (busid != EKitBus::BUS_I2C_FIRMWARE) {
			throw EKitException(func_name, "Not compatible bus passed: EKitBus::BUS_I2C_FIRMWARE is required");
		}
	}
