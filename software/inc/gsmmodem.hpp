...
private:
    ...
    GSMModem(std::shared_ptr<EKitBus>& ebus, int addr, const std::string& name);
...
public:
    ...
    /// \param name - name of the GSM modem
    GSMModem(std::shared_ptr<EKitBus>& ebus, int addr, int timeout_ms, const std::string& name);
