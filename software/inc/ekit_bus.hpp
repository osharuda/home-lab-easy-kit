...
public:
	...
	virtual EKIT_ERROR open() = 0;

    /// \brief Close a bus.
    /// \return Corresponding EKIT_ERROR error code.
	virtual EKIT_ERROR close() = 0;
	...
	virtual EKIT_ERROR lock(int address)=0;

    /// \brief Unlock a bus.
    /// \return Corresponding EKIT_ERROR error code.
    /// \note Once bus is unlocked, any other thread may lock it again.
	virtual EKIT_ERROR unlock()=0;
