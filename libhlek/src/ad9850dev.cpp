...
void AD9850Dev::update(double frequency, double phase) {
    ...
    uint32_t f = static_cast<uint32_t>((frequency / clock_freq) * static_cast<double>(UINT32_MAX));
    ...
    uint8_t p = static_cast<uint8_t>( (phase / (2.0*M_PI)) * static_cast<double>(0b00011111));
    assert(p < 0b00100000);
    cmd.phase = p;
