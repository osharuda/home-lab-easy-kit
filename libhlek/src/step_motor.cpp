...
StepMotorDev::StepMotorDev(std::shared_ptr<EKitBus>& ebus, const StepMotorConfig* cfg) :
...
size_t StepMotorDev::get_motor_count() const {
	return config->motor_count;
}

std::vector<const StepMotorDescriptor*> StepMotorDev::get_motor_info() const {
	std::vector<const StepMotorDescriptor*> res;
	res.resize(config->motor_count);
	for (size_t mindex=0; mindex<config->motor_count; mindex++) {
		res[mindex] = config->motor_descriptor[mindex];
	}
	return res;
}
...
uint8_t StepMotorDev::microstep_divider(size_t mindex) {
    uint8_t driver_type = config->motor_descriptor[mindex]->motor_driver;
...
uint64_t StepMotorDev::speed(size_t mindex, double value, bool rpm) {
	...
	if (rpm) {
        ...
        double spr = (double)(config->motor_descriptor[mindex]->steps_per_revolution);
...
uint8_t StepMotorDev::status(std::vector<StepMotorStatus>& mstatus) {
    ...
    mstatus.resize(config->motor_count);
...
void StepMotorDev::clear() {
	...
	for (size_t i=0; i<mcount; i++) {
		motors_data[i].speed = config->motor_descriptor[i]->default_speed;
		motors_data[i].microstep = STEP_MOTOR_MICROSTEP_STATUS_TO_VALUE(config->motor_descriptor[i]->config_flags);
	}
}
