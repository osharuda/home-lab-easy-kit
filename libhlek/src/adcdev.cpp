...
std::string ADCDev::get_input_name(size_t index, bool channel_name) const {
	...
	return channel_name ? config->inputs[index].adc_input : config->inputs[index].in_name;
}
...
void ADCDev::start(uint16_t sample_count, double delay_sec){
	...
	if (delay_sec==0) {
		...
		int res = tools::stm32_timer_params(config->timer_freq, delay_sec, data.timer_prescaller, data.timer_period, expected);
		if (res>0) {
